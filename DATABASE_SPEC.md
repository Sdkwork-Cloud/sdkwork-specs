...
- 版本�?.0
- 定位：跨语言、跨框架、跨 ORM、跨数据库产品的表结构定义标�?
- 适用对象：后端服务、数据平台、SDK 生成器、DDL/结构变更脚本、审计工具、数据库设计评审

本文档定义一套通用数据库表结构标准。它不绑�?Java、Spring、JPA、Hibernate、Rust、Python、TypeScript、Go、PHP、C# 或任何单一技术栈，也不要求所有应用使用同一�?ORM 或同一套基础类。它的目标是把表结构抽象成稳定的数据契约，使不同语言、不同框架、不同数据库之间可以共享数据语义、生�?SDK、拆分服务、替换数据库并长期演进�?

本文以关系数据库为主契约来源，同时给出文档库、列式库、搜索索引、事件流、读模型的派生规则。任何工程中的实体基类、历�?DDL、结构变更脚本或代码生成器，只能作为兼容映射参考；通用规范应以本文定义的字段语义、逻辑类型、约束、索引、结构演进规则和跨语言序列化规则为准�?
...
| 等级 | 名称 | 适用场景 | 最低要�?|
| --- | --- | --- | --- |
| L0 | Legacy Compatible | 存量系统过渡 | 标准字段映射表、兼容边界、风险说�?|
| L1 | Portable Core | 新业务表最低标�?| 标识、审计、命名、逻辑类型、基础索引 |
| L2 | Service Ready | 多服务共享或拆分 | 多租�?归属、幂等、API 序列化、结构演进治�?|
| L3 | Enterprise Grade | 金融、支付、合规、高可用 | 审计、隐私、生命周期、事件一致性、自动化检�?|

新建核心业务�?MUST 达到 L1。多租户、支付、账户、权限、消息、第三方回调、跨服务写入�?MUST 达到 L2。涉及资金、凭证、隐私、法务留存、关键审计的�?SHOULD 达到 L3�?

### 1.1 标准产物

任何采用本规范的项目 SHOULD 形成以下标准产物，而不是只在某个语言模型�?ORM 注解中隐式表达：

| 产物 | 要求 |
| --- | --- |
| 表结构契�?| 记录表名、字段、逻辑类型、约束、索引、owner、画像和合规等级 |
| 业务模块前缀注册�?| 记录允许使用�?`<module_prefix>`、业务边界、owner 和示�?|
| 公共字段字典 | 记录 `id`、`uuid`、`tenant_id`、`created_at` 等公共字段唯一语义 |
| 逻辑类型映射�?| 记录逻辑类型�?PostgreSQL、MySQL、SQLite、SQL Server、Oracle 等数据库的映�?|
| 跨语言序列化规�?| 记录 `int64`、decimal、instant、enum、json �?API/SDK 中的表达 |
| 自动化检查规�?| 记录 schema linter、CI、代码生成器需要检查的规则编号 |
| 例外登记�?| 记录历史兼容、工具元数据表、临时表、外�?raw 表等例外原因和退出条�?|

最低落地要求：

- 新表设计 MUST 先写契约，再生成或校�?DDL、ORM、DTO、SDK�?
- 新业务模�?MUST 先登记前缀，再创建表�?
...
- SDK/API 生成：数据库字段稳定映射�?OpenAPI、GraphQL、gRPC、TypeScript SDK、Python SDK、Rust DTO、Go client、C# client�?
...
- 不规定必须使�?PostgreSQL、MySQL 或任一数据库�?
- 不要求所有表都建立数据库外键�?
- 不要求所有服务使用同一套基础类�?
- 不要求所有业务都暴露内部主键�?
- 不要求为了“通用”牺牲明确的业务约束�?
...
1. **数据契约优先**：字段名、类型、约束和语义�?ORM 注解或语言类名更稳定�?
2. **逻辑类型先行**：先定义 `int64`、`instant`、`decimal`、`json` 等逻辑类型，再映射到具体数据库类型�?
3. **跨语言安全**：`int64`、`decimal`、时间、枚举、JSON �?API 表达必须避免精度和时区错误�?
4. **隔离条件显式**：租户、组织、用户、owner、数据范围不能只靠口头约定�?
5. **变更可灰�?*：任何破坏性结构变更必须经过扩展、兼容、校验、切换、收缩，不得只靠一次性脚本假设成功�?
6. **核心查询列化**：高频查询、唯一约束、排序、权限过滤字�?MUST 是独立列，不能只藏在 JSON 中�?
7. **可审计可追踪**：核心业务数据必须能回答谁创建、何时创建、属于谁、为何变化、如何重放或补偿�?
8. **历史兼容可映�?*：存量字段可以短期保留，但必须有标准字段映射、兼容边界和退出说明�?
9. **数据库能力分�?*：可以使用数据库特性优化，但公共契约不能依赖不可移植的专有特性�?
10. **工具可检�?*：规范必须能�?schema linter、结构变更审查、代码生成器�?CI 部分自动验证�?

## 4. 可移植数据契�?

通用表结构不直接等于某个数据�?DDL，也不等于某个语言的实体类。标准的中间层称为可移植数据契约，包含：

| 契约元素 | 必须描述 |
| --- | --- |
| `table_name` | 标准表名、业务域、所有者团�?|
| `table_profile` | 表画像，例如业务实体、日志、账本、读模型 |
| `columns` | 字段名、逻辑类型、长�?精度、空值策略、默认�?|
| `constraints` | 主键、唯一键、检查约束、外键或应用级引�?|
| `indexes` | 查询目标、字段顺序、唯一性、部分索引条�?|
| `ownership` | 租户、组织、用户、owner、平台共享数据规�?|
| `lifecycle` | 创建、更新、删除、归档、留存、清理策�?|
| `serialization` | API/SDK 字段名、`int64`、decimal、时间、枚举表�?|
| `evolution` | 契约版本、兼容期、弃用策略、校验方式、收缩条�?|
| `security` | 敏感字段、加密、脱敏、访问审�?|

推荐�?YAML、JSON、DSL、注解扫描结果或结构变更清单维护该契约。DDL、ORM 模型、SDK DTO、OpenAPI schema �?SHOULD 从契约生成或经过契约校验�?
...
每张�?MUST 明确事实来源和写入所有者：

| 属�?| 说明 |
| --- | --- |
| `system_of_record` | 是否为事实来源。主表、账本、审计日志通常�?true；读模型、搜索索引通常�?false�?|
| `write_owner` | 允许写入该表的服务、任务或数据管道�?|
| `read_consumers` | 依赖该表的主要服务、报表、搜索、数仓或 SDK�?|
| `change_channel` | 变更传播方式，例�?CDC、outbox、批处理、API 同步�?|
...
大型系统 SHOULD 维护 schema registry，用于管理表契约、字段字典、结构演进状态和 SDK 映射。schema registry 可以是代码仓库中�?YAML/JSON 文件、数据库元数据表、OpenAPI 扩展、数据目录系统或内部平台�?

schema registry 至少 SHOULD 支持�?

- 查询字段语义、敏感等级、逻辑类型和序列化方式�?
- 校验 DDL 与契约是否一致�?
- 生成或校�?ORM 模型、DTO、OpenAPI、GraphQL、gRPC schema�?
- 记录破坏性变更、兼容窗口和演进进度�?
- 标识事实来源、读模型、数�?owner 和下游消费者�?
...
| 概念 | 标准要求 |
| --- | --- |
| 业务�?| 每张核心�?MUST 归属一个业务域，例�?iam、commerce、content、notification |
| bounded context | 跨服务共享语义前 MUST 明确上下文，避免同名字段在不同领域含义不�?|
| aggregate root | 强一致写�?SHOULD 围绕聚合根设计，聚合内使用事务，聚合间使用事�?|
| shared kernel | 多服务共享字段必须进入公共契约，不能靠复制粘�?DTO 维持一�?|
| anti-corruption layer | 外部系统字段 SHOULD 通过适配层映射到内部标准字段 |

规则�?

- 表名、字段名、枚举名 SHOULD 能反映所属业务域，避�?`common_data`、`system_record`、`business_info` 这类无边界名称�?
- 一个字段在不同业务域含义不同时，MUST 使用不同字段名或在契约中标注上下文，不能用同一�?`type`、`status`、`source` 承载多套含义�?
- 跨服�?join 不应成为核心在线路径。需要跨服务查询时，SHOULD 通过读模型、搜索索引、API composition 或事件投影实现�?
...
| 设计 | 适用场景 | 风险 |
| --- | --- | --- |
| 规范�?| 主数据、配置、权限、强一致引�?| join 增多、跨服务拆分困难 |
| 反规范化 | 列表读模型、搜索、报表、历史快�?| 冗余字段漂移 |
| 快照 | 订单、合同、支付请求、审计事�?| 快照字段版本膨胀 |
| 宽表 | 报表、列表页、离线分�?| 写入成本高、字段治理难 |
| EAV | 动态属性、低频扩�?| 查询和约束弱，不能承载核心字�?|

规则�?

- 事实数据 SHOULD 保存发生当时的关键快照，例如订单商品名、价格、买家信息，避免历史事实随主数据变化�?
- 快照字段 MUST �?schema version，并能说明来源对象和采集时间�?
- 读模型中的冗余字�?MUST 有重建路径和漂移校验�?
- EAV 模型 MUST NOT 用于金额、状态、租户、权限、幂等等核心字段�?
- 反规范化字段 SHOULD 在名称或契约中标注来源，例如 `seller_name_snapshot`、`product_title_snapshot`�?

## 5. 表画�?

表画像决定必备字段、索引、约束、写入策略和结构演进要求。新�?MUST 先选择一个主画像，可以叠加多个附加画像�?
...
| `relation_entity` | 用户角色、订单商品、标签关�?| 标识、审计、关系唯一�?|
| `dictionary_entity` | 字典、配置、渠道、枚举扩�?| 标识、code、状态、版�?|
| `event_log` | 回调、任务、操作记录、异步事�?| 标识、幂等、payload、处理状�?|
| `ledger_entry` | 账户流水、余额变化、积分流�?| 标识、账本方向、金额、幂等、不可变 |
| `audit_log` | 安全审计、后台操作、合规追�?| 标识、操作者、目标对象、请求链路、不可变 |
| `outbox_event` | 本地事务后发布事�?| 标识、聚合对象、事件类型、payload、发布状�?|
| `inbox_event` | 消费方去重和处理记录 | 标识、消息来源、幂等键、处理状�?|
| `projection` | 读模型、列表缓存、聚合视�?| 来源对象、同步版本、重建标�?|
| `snapshot` | 历史快照、订单快照、配置快�?| 来源对象、版本、快照内�?|
| `temporary` | 临时导入、短期缓存、中间结�?| 标识、过期时间、清理策�?|

### 5.2 字段�?

| 字段�?| 字段 | 说明 |
| --- | --- | --- |
| `identity` | `id`, `uuid` | 内部关联和外部稳定标�?|
| `audit` | `created_at`, `updated_at`, `version` | 创建、更新、乐观锁 |
| `actor_audit` | `created_by`, `updated_by`, `operator_id` | 操作人追�?|
| `tenant_scope` | `tenant_id`, `organization_id`, `data_scope` | 租户和组织隔�?|
| `user_scope` | `user_id` | 用户私有或用户归�?|
| `owner_scope` | `owner_type`, `owner_id` | 多主体归�?|
| `lifecycle` | `status`, `deleted_at`, `deleted_by`, `archived_at` | 生命周期和软删除 |
| `tree` | `parent_id`, `parent_uuid`, `path`, `level_no`, `sort_order` | 树形结构 |
| `idempotency` | `request_id`, `idempotency_key`, `external_event_id`, `payload_hash` | 幂等与重�?|
| `trace` | `trace_id`, `span_id`, `source_system`, `source_version` | 链路追踪和来�?|
| `retention` | `expire_at`, `retention_until`, `legal_hold` | 留存、TTL、法务冻�?|
...
| `relation_entity` | `id`, `uuid`, 两端引用 ID, 关系唯一�? `created_at` |
| `dictionary_entity` | `id`, `uuid`, `code`, `name`, `status`, `created_at`, `updated_at`, `version` |
| `event_log` | `id`, `uuid`, `created_at`, `status`, `idempotency_key` �?`external_event_id`, `payload_hash` |
...
| `uuid` | `string(36..64)` | SHOULD | 外部稳定标识，用�?API、同步、补偿、跨库引�?|
| `code` | `string(32..128)` | MAY | 业务编码、字典编码、可读引�?|
| `business_no` | `string(32..128)` | MAY | 业务单号，例如订单号、支付单�?|

规则�?

- `id` MUST 使用 64 位整数或可稳定映射到 64 位的内部 ID。推荐雪�?ID、号段服务、数据库序列批量领取、ULID/UUID 派生策略。新系统 SHOULD NOT 依赖单库自增 ID 作为跨服务主键策略�?
- `uuid` SHOULD 全局唯一，推�?UUID v7、UUID v4、ULID、KSUID 或组织统一字符�?ID。对�?API 优先暴露 `uuid`、`code` �?`business_no`，不要默认暴露内部自�?ID�?
...
| `version` | `int64` | MUST | 乐观锁或数据版本，初�?0 |
| `created_by` | `int64` | SHOULD | 创建�?|
| `updated_by` | `int64` | SHOULD | 最近更新人 |

规则�?

- 新表 MUST 使用 `created_at`、`updated_at`，不要使�?`created_time`、`update_time`、`gmt_create` 等变体作为标准字段�?
- 存储精度 SHOULD 至少毫秒，L3 �?SHOULD 支持微秒�?
- 审计时间 MUST 由服务端或数据库端可信时间生成。客户端传入时间只能作为业务时间字段，不能覆盖审计时间�?
- `version` MUST 在并发更新时参与条件更新，或有等价的数据版本机制�?
- 存量系统使用短字�?`v` 时，可在兼容期保留，但契约层 MUST 映射�?`version`�?
...
| `organization_id` | `int64` | 多组织表 MUST | 组织 ID，没有组织时�?0 |
| `workspace_id` | `int64` | MAY | 工作�?ID |
| `project_id` | `int64` | MAY | 项目 ID |
| `data_scope` | `int32` �?enum | 多租户表 SHOULD | 数据可见范围 |
...
- 跨租户引�?MUST 同时校验 `tenant_id` 或声明为平台共享引用�?
...
| `owner_type` | `string(32)` �?`int32` | 多主体归�?MUST | 主体类型 |
| `owner_id` | `int64` | 多主体归�?MUST | 主体 ID |
| `operator_id` | `int64` | 操作记录 SHOULD | 当前操作�?|
| `operator_type` | `string(32)` �?`int32` | MAY | USER、SYSTEM、ADMIN、JOB、SERVICE |

规则�?

- `owner_type/owner_id` 用于资源可能归属于用户、组织、租户、应用、项目、服务账号等不同主体的场景�?
- `owner_type` 若对外暴�?SHOULD 使用字符串枚举，例如 `USER`、`ORG`、`TENANT`、`APP`、`PROJECT`。高频内部表 MAY 使用整数枚举，但必须有稳定映射�?
...
| 字段 | 逻辑类型 | 说明 |
| --- | --- | --- |
| `status` | `int32` �?`string(32)` | 业务生命周期状�?|
| `deleted_at` | `instant` | 软删除时�?|
| `deleted_by` | `int64` | 删除�?|
| `archived_at` | `instant` | 归档时间 |
| `expire_at` | `instant` | 过期时间 |
| `retention_until` | `instant` | 最短留存到期时�?|
| `legal_hold` | `boolean` | 法务冻结，禁止清�?|

规则�?

- 核心业务�?SHOULD �?`status` 表达生命周期。`is_deleted` 只适合极简单软删除，不适合复杂状态机�?
- 有审计要求的删除 MUST 记录 `deleted_at` �?`deleted_by`，或写入不可变审计日志�?
- 物理删除 MAY 用于临时表、缓存表、过期中间数据、测试数据或法规要求的删除，但必须明确影响范围�?
- 清理任务 MUST 遵守 `retention_until` �?`legal_hold`�?

### 6.6 树形字段

| 字段 | 逻辑类型 | 说明 |
| --- | --- | --- |
| `parent_id` | `int64` | 父节点内�?ID，根节点�?0 |
| `parent_uuid` | `string(64)` | 父节点外�?ID，根节点�?`0` |
| `path` | `string(1000..4000)` | 祖先路径，可�?`/1/2/3/` |
| `level_no` | `int32` | 层级，根节点�?0 |
| `sort_order` | `int32` | 同级排序 |

规则�?

- 树形�?MUST 避免仅依赖递归查询作为唯一读取路径。高频场�?SHOULD 维护 `path` 或闭包表�?
- 跨库同步、API 或离线导入时 SHOULD 使用 `parent_uuid`�?
- `parent_id > 0` �?SHOULD 同时提供 `parent_uuid`，避免跨�?ID 重映射时丢失父子关系�?

### 6.7 幂等和链路字�?

| 字段 | 逻辑类型 | 说明 |
| --- | --- | --- |
| `request_id` | `string(64..128)` | 请求 ID，贯穿链�?|
| `trace_id` | `string(64..128)` | 分布式追�?ID |
| `idempotency_key` | `string(128..200)` | 幂等�?|
| `external_event_id` | `string(128..255)` | 第三方事�?ID |
...
| 场景 | 推荐策略 |
| --- | --- |
| 必须存在的业务字�?| `NOT NULL`，由应用或数据库默认值填�?|
| 未知�?| 允许 `NULL`，但必须说明未知来源 |
| 不适用 | 优先使用单独状态或类型字段说明，不要滥�?`NULL` |
| 数量和金�?| 通常 `NOT NULL DEFAULT 0`，但要区分“无记录”和“零值�?|
| 租户级组织字�?| `organization_id NOT NULL DEFAULT 0` |
| 根节�?| `parent_id NOT NULL DEFAULT 0`、`parent_uuid NOT NULL DEFAULT '0'` |

规则�?

- `NULL`、空字符串�?、`UNKNOWN` 不能混用表达同一个含义�?
...
生产�?SHOULD 有列注释或外部数据字典。注释不是展示文案，而是数据语义说明�?
...
| ID 类型 | 用�?| 推荐策略 |
| --- | --- | --- |
| `id` | 内部主键、join、审计引�?| 默认 64 位正整数雪花 ID，由应用/服务统一 ID Provider 生成 |
| `uuid` | 外部引用、同步、API、事�?| UUID v7、UUID v4、ULID、KSUID 或组织统一字符�?ID |
| `business_no` | 业务单号、人工排障、对�?| 业务域前缀 + 时间/序列/随机段，避免泄露规模 |
| `external_*_id` | 第三方系�?ID | provider + 外部 ID 联合唯一 |

默认雪花 ID profile�?
| 字段 | 标准�?| 说明 |
| --- | --- | --- |
| 符号�?| 1 bit 保留�?0 | ID MUST 为正 `int64`，避免跨语言有符号整数问�?|
| 时间�?| 41 bit 毫秒 | �?SDKWork 默认 epoch 起算，可使用�?69.7 年，满足 30 年以上不重复要求 |
| 节点 ID | 10 bit | 单个部署唯一节点号，范围 `0..1023` |
| 毫秒序列 | 12 bit | 单节点每毫秒 `4096` �?ID |
| 默认 epoch | `2024-01-01T00:00:00Z` | Rust `sdkwork_platform_id_service::default_snowflake_epoch_millis()` 的公开�?|
| 最低寿�?| `30` �?| 自定�?profile MUST 提供容量证明且不得低于该�?|
...
id_strategy:
  ...
  expected_lifetime_years: 69
  node_id_source: deployment_config
  clock_rollback: reject_and_alert
  sequence_overflow: wait_next_millisecond_or_fail_closed
  restart_recovery: persist_or_monotonic_state_guard
  public_id: uuid
```

规则�?
- ID 生成�?MUST 明确节点 ID、时钟回拨、序列溢出、重启恢复、容量证明、冲突检测和监控策略�?- 运行时业务数�?INSERT �?`BIGINT id` MUST 由统一 ID Provider 显式生成并绑定写入；仓储、DAO、ORM repository、SQL mapper、后台任务、事件消费者和迁移后的写入路径不得各自临时实现取号逻辑�?- SDKWork Rust 实现 MUST 优先复用 `sdkwork-appbase/crates/sdkwork-platform-id-service` 中的 `sdkwork_platform_id_service::SnowflakeIdGenerator`；其他语言实现应在 appbase 对应包中提供等价能力，且遵守同一 profile�?- 写入框架 MUST 在进入数据库前准�?`id`、`uuid`、审计时间、租�?组织/用户上下文和幂等键；业务服务只能依赖该写入框架或注入�?ID Provider，不能在 SQL 片段中隐�?ID 分配�?- 运行时业务表 DDL MUST 使用 `id BIGINT NOT NULL PRIMARY KEY` 或数据库等价的非自动分配 `int64` 主键；SQLite 不得使用 `INTEGER PRIMARY KEY` 作为运行时业�?ID，因为它会回落到 rowid 自动分配语义�?- 运行时业务数�?INSERT MUST 显式列出 `id` 列并绑定 ID Provider 生成的值。`RETURNING id` 只允许返回已经绑定的 ID，不得作为数据库分配 ID 的证据�?- 运行时业务数�?INSERT MUST NOT 依赖 `BIGSERIAL`、`SERIAL`、`AUTOINCREMENT`、`GENERATED ... AS IDENTITY`、`DEFAULT nextval(...)`、单条数据库 `RETURNING id`、`last_insert_rowid()`、`MAX(id)+1` 或本地哈希派�?ID 作为主键分配策略；历史表、临时测试表、外�?raw 落地表和已登记迁移窗口中的兼容表除外�?- 节点 ID MUST 由部署配置、编排平台、实例注册表或受控本地配置提供；生产环境不得在进程启动时随机选择节点 ID。测试环境可以使用保留节点区间，但不得泄漏到生产配置�?- 时钟回拨 MUST 失败关闭或在明确上限内等待后失败并告警；不得在观察到回拨后继续生成早于上一成功时间戳的 ID�?- 序列溢出 SHOULD 等待下一毫秒；如果运行时无法等待，MUST 失败关闭并记录可观测错误�?- 自定�?ID 策略 MAY 使用不同 bit 分配、号段或外部 ID 服务，但 MUST 在契约中声明 `type`、适用�?范围、容量和寿命证明、节�?号段冲突防护、时�?溢出/重启策略、失败关闭行为、观测指标、跨语言序列化和回滚方案。未证明 30 年以上容量的自定义策略不得作为默认运行时业务 ID�?- 官方安装种子、内置目录、标准配置和需要被 `target_id` 稳定引用的数�?MUST 使用保留稳定 ID 或稳�?UUID，不得改用运行时雪花 ID，否则会破坏安装幂等、引用修复和跨版本升级�?- 雪花 ID 或时间有�?ID 在多区域部署�?MUST 处理 clock rollback，不能假设机器时钟永远单调�?- 对外公开 ID SHOULD 使用 `uuid`、ULID、KSUID 或业务单号，不直接暴露连续自�?ID�?- 业务单号不能只由时间戳组成，必须考虑并发、重试、预测风险和人工录入校验�?
...
- 关联�?SHOULD 使用两端实体名并保留业务模块前缀：`iam_role_permission`、`commerce_order_item`�?- 新建业务表的第一�?MUST 是业务模块前缀，格式为 `<module_prefix>_<entity_name>`�?
...
- 随意缩写：`usr`、`ord_itm`�?
- 使用数据库保留字。若业务名是 `order`，落地时使用 `commerce_order`、`sales_order` �?`commerce_order_record`�?
...
| 表名 | 模块前缀 | 说明 |
| --- | --- | --- |
| `iam_user` | `iam` | 身份和访问管理用�?|
| `iam_role_permission` | `iam` | 权限关系�?|
...
| `studio_project_content` | `studio` | 设计时项目内�?|
...
- 跨模块共享表必须选择事实来源模块前缀。例如用户角色属�?`iam_`，不应命名为 `user_role` �?`plus_user_role`�?
...
| 前缀 | 业务�?| 示例�?|
| --- | --- | --- |
| `iam` | 身份、租户、组织、RBAC、API key、安全策�?| `iam_user`、`iam_tenant`、`iam_role_permission` |
| `commerce` | 订单、支付、退款、账户、优惠券、VIP、商品、发�?| `commerce_order`、`commerce_payment`、`commerce_coupon` |
| `ai` | Agent、模型、提示词、生成任务、AI 工具 | `ai_agent`、`ai_model_info`、`ai_prompt` |
| `content` | 文章、评论、收藏、标签、分享、详情页、投�?| `content_article`、`content_comment` |
| `media` | 文件、图片、视频、语音、音乐、数字人、媒体发布记�?| `media_file`、`media_video` |
| `comms` | 会话、消息、IM、RTC、话�?| `comms_conversation`、`comms_message` |
| `data` | 数据源、schema、table、column、向量库、知识库、记�?| `data_datasource`、`data_vector_store` |
| `ops` | 调度、通知、邮件、反馈、访问记录、短链、网络、爬�?| `ops_schedule_task`、`ops_notification` |
| `integration` | 外部渠道、连接器、供应商账号、代理配�?| `integration_channel`、`integration_channel_account` |
| `studio` | 工作空间、项目、应用、文档页、模板等设计时资�?| `studio_workspace`、`studio_project`、`platform_app` |
| `game` | 游戏、房间、比赛、积分、排行榜 | `game_room`、`game_leaderboard` |
| `recruit` | 招聘、简历、职位、投递、人才池 | `recruit_resume`、`recruit_delivery` |

现有历史表如果已经使用项目级前缀，例�?`plus_*`，只能作�?L0 legacy compatible 命名。新�?MUST 使用业务模块前缀。本文当前目标是制定标准和标注合规差距，不要求本轮执行物理表 rename；如果未来另行立项做物理改名，必须单独输出变更方案�?

模块归属判定优先级：

1. 先判定事实来�?owner，而不是当前代码包名或服务名�?
2. 表达身份、租户、组织、权限、API key、安全策略的表归�?`iam_`�?
3. 表达订单、账户余额、支付、退款、商品、优惠券、会员、发票、佣金伙伴的表归�?`commerce_`�?
4. 表达 Agent、模型、提示词、AI 工具、技能、插件、生成配置、AI 调用/用量事实的表归入 `ai_`�?
5. 表达外部渠道、连接器、供应商账号、代理配置的表归�?`integration_`，不要因为它�?AI 使用就归�?`ai_`�?
6. 表达工作空间、项目、应用、文档页、PPT 模板等设计时资产的表归入 `studio_`，不要使�?`app_`、`project_` 作为全局前缀�?
7. 表达二进�?媒体资源、媒体发布记录、文件分片、图片、视频、音频、数字人的表归入 `media_`�?
...
7. 如果�?outbox、inbox、audit、ledger、projection 等技术画像，仍然必须带业务模块前缀，例�?`commerce_outbox_event`、`iam_audit_log`�?

不合规示例：

| 表名 | 问题 | 建议 |
| --- | --- | --- |
| `plus_user` | `plus` 是产品名，不是业务模�?| `iam_user` |
| `app_order` | `app` 是应用层，不是业务域 | `commerce_order` |
| `sys_config` | `sys` 语义过泛 | `ops_config` 或具体模块配置表 |
| `common_dict` | `common` �?owner | `iam_dictionary` �?`ops_dictionary` |
...
| 字段 | 要求 |
| --- | --- |
| `prefix` | 小写字母开头，仅允许小写字母、数字，推荐 2 �?16 个字�?|
| `domain_name` | 业务域名称，例如 Identity Access、Commerce、Studio |
| `owner_team` | 负责该业务域表结构标准的团队�?owner |
| `bounded_context` | 该前缀覆盖的上下文边界 |
| `write_policy` | 单写�?owner、多写入网关、事件写入或离线写入策略 |
| `table_examples` | 至少 3 个代表性标准表�?|
| `forbidden_aliases` | 不允许作为该域别名的前缀，例�?`app`、`sys`、`common` |
| `status` | `ACTIVE`、`DEPRECATED`、`RESERVED` |
| `valid_from` | 前缀生效版本或日�?|

前缀注册规则�?

- 新前缀 MUST 通过数据架构评审，不能由单个应用、单个语言仓库或单�?ORM 模块自行引入�?
- 一个业务域 SHOULD 只有一个主前缀；确需拆分时，必须说明新旧边界和事实来源变化�?
- 前缀一旦发�?MUST NOT 复用为其它含义。废弃前缀只能标记�?`DEPRECATED` �?`RESERVED`�?
- `app`、`web`、`api`、`service`、`server`、`admin`、`backend`、`frontend`、`java`、`python`、`rust`、`ts`、`mysql`、`pg`、`db`、`sys`、`common`、`base`、`core` 不得作为默认业务前缀�?
- 产品名、公司名、项目名、客户名、租户名、环境名不进入表名前缀；这些信息放�?database/schema/catalog、部署配置、租户字段或数据目录中�?
...
| 表型 | 格式 | 示例 | 说明 |
| --- | --- | --- | --- |
| 主实�?| `<module>_<entity>` | `iam_user`、`commerce_order` | 领域内核心事实对�?|
| 子实�?| `<module>_<parent>_<child>` | `commerce_order_item` | 生命周期依赖父实�?|
| 关系�?| `<module>_<left>_<right>` | `iam_role_permission` | 关系事实属于该模�?|
| 历史�?| `<module>_<entity>_history` | `commerce_account_history` | 可变对象的历史轨�?|
| 账本�?| `<module>_<entity>_ledger` | `commerce_account_ledger` | 不可变资金或额度流水 |
| 事件�?| `<module>_<entity>_event` | `commerce_payment_event` | 领域事件或业务事�?|
| 回调�?| `<module>_<provider>_<event>` | `commerce_stripe_webhook_event` | 第三方回调需�?provider 边界 |
| 快照�?| `<module>_<entity>_snapshot` | `content_document_snapshot` | 某一时点状�?|
| 投影�?| `<module>_<entity>_projection` | `comms_conversation_projection` | 读模型或查询优化 |
| Outbox | `<module>_outbox_event` | `commerce_outbox_event` | 模块内事务消�?|
| Inbox | `<module>_inbox_event` | `integration_inbox_event` | 模块内消费幂�?|
| 字典�?| `<module>_<entity>_dictionary` | `iam_security_dictionary` | 有明�?owner 的枚�?字典 |
| 临时�?| `<module>_<entity>_staging` | `data_import_staging` | 导入、清洗或过渡数据 |
| 缓存�?| `<module>_<entity>_cache` | `content_feed_cache` | 可重建缓存，必须声明 TTL |

命名细则�?

- 表名 SHOULD 控制�?63 个字符以内，以兼�?PostgreSQL 等默认标识符长度；超过时应缩短上下文而不是丢失业务含义�?
- 实体�?SHOULD 使用单数：`user`、`order`、`message`；列表和集合语义通过关系或子实体表达�?
- `record`、`data`、`info`、`detail` 不能单独作为实体名；只有在明确业务对象后才能作为后缀，例�?`invoke_record`、`order_detail`�?
- 第三方来源表必须�?provider �?external 边界，例�?`integration_slack_channel`、`commerce_stripe_webhook_event`�?
...
- 数据库系统表、ORM 内部表、结构变更工具元数据�?MAY 使用工具约定名称，例�?`flyway_schema_history`、`databasechangelog`�?
- 临时实验�?MAY 使用 `tmp_` 前缀，但不得进入生产契约，不得被业务代码长期依赖�?
- 外部系统原始落地�?MAY 使用 `<module>_<provider>_<entity>_raw`，但必须声明清洗后的标准目标表�?
- 多租户物理分�?MAY 在表名后追加分片后缀，例�?`commerce_order_202604`，但逻辑契约表名仍是 `commerce_order`�?

禁止例外�?

- 不得因为表由 Java、Python、Rust、TypeScript 服务创建而使用语言前缀�?
- 不得因为表位�?backend、admin、server、api 工程而使用应用层前缀�?
- 不得�?`common_` 承接所有跨模块数据；跨模块表必须选择事实来源 owner�?
...
- 时间点使�?`_at`：`published_at`、`processed_at`�?
- 持续时长带单位：`duration_ms`、`timeout_seconds`�?
- 金额使用 `_amount`，币种使�?`currency`�?
...
| 普通索�?| `idx_` | `idx_commerce_payment_tenant_status_created` |
| 外键 | `fk_` | `fk_order_user` |
| 检查约�?| `chk_` | `chk_payment_amount_non_negative` |
| JSON 索引 | `gin_`、`idx_` | `gin_product_metadata` |
| 全文索引 | `fts_` | `fts_article_content` |

规则�?

- 索引�?SHOULD 表达业务字段，不使用 ORM 自动生成的随机名称作为标准名�?
- 名称长度受限的数据库可以缩短，但 MUST 保持可读和可追踪�?
- 结构变更脚本中的约束�?MUST 稳定，便于回滚和跨环境比对�?
...
| `int32` | `INTEGER` | `INT` | `INTEGER` | `INT` | `NUMBER(10,0)` | `Int32` | 普通整�?|
| `string(n)` | `VARCHAR(n)` | `VARCHAR(n)` | `TEXT` | `NVARCHAR(n)` | `VARCHAR2(n)` | `String` | UTF-8 字符�?|
| `text` | `TEXT` | `TEXT` | `TEXT` | `NVARCHAR(MAX)` | `CLOB` | `String` | 大文�?|
| `boolean` | `BOOLEAN` | `BOOLEAN` �?`TINYINT(1)` | `INTEGER` 0/1 | `BIT` | `NUMBER(1,0)` | `Bool` | `bool` |
| `instant` | `TIMESTAMPTZ` | `DATETIME(6)` UTC | ISO8601 `TEXT` �?epoch ms | `DATETIMEOFFSET` | `TIMESTAMP WITH TIME ZONE` | `DateTime64` UTC | ISO 8601 UTC |
| `date` | `DATE` | `DATE` | ISO8601 `TEXT` | `DATE` | `DATE` | `Date` | 日期，不带时�?|
| `decimal(p,s)` | `NUMERIC(p,s)` | `DECIMAL(p,s)` | `TEXT` 或最小单位整�?| `DECIMAL(p,s)` | `NUMBER(p,s)` | `Decimal(p,s)` | JSON 字符�?|
| `json` | `JSONB` | `JSON` | `TEXT` | `NVARCHAR(MAX)` + 校验 | `JSON` �?`CLOB` | `String` �?`JSON` | �?schema 的对�?|
| `bytes` | `BYTEA` | `BLOB` | `BLOB` | `VARBINARY(MAX)` | `BLOB` | `String` base64 | byte array |
| `uuid_string` | `UUID` �?`VARCHAR(64)` | `VARCHAR(64)` | `TEXT` | `UNIQUEIDENTIFIER` �?`NVARCHAR(64)` | `VARCHAR2(64)` | `String` | 字符串契�?|
| `geo_point` | PostGIS `geometry(Point,4326)` | `POINT SRID 4326` | `longitude/latitude` 双列 | `geography` | Spatial | tuple | `{lat,lng}` |
| `vector` | `vector(n)` 扩展 | 专用扩展�?BLOB | BLOB/TEXT | 专用扩展 | 专用扩展 | Array(Float32) | 派生能力，不作核心契�?|
...
| --- | --- |
| 普通业务文�?| UTF-8，保留原�?|
| 登录名、邮箱、手机号 | 保存原文，同时保存规范化�?|
| 大小写不敏感唯一 | 使用 `*_normalized` 列或数据库明确的 case-insensitive 类型 |
| 多语言排序 | 不依赖数据库默认排序，必要时使用 locale-aware 排序�?|
| URL、code、业务单�?| 通常使用大小写敏感比较，除非业务明确不敏�?|

规则�?

- 新系�?MUST 使用 UTF-8 或等�?Unicode 字符集�?
- 不能假设 MySQL、PostgreSQL、SQLite、SQL Server、Oracle 的默认大小写敏感性一致�?
- 邮箱、用户名、手机号等登录标�?SHOULD 使用 `email_normalized`、`username_normalized`、`phone_e164` 等规范化列建立唯一键�?
- Unicode 文本 SHOULD 统一规范化形式，例如 NFC，避免肉眼相同但字节不同导致唯一性绕过�?
- 模糊搜索、拼音搜索、分词搜索不应直接依�?OLTP 主表�?`LIKE '%keyword%'`，应使用搜索索引、全文索引或投影列�?
- 跨库移植�?MUST 检查目标数据库排序规则是否会导致原本可共存的数据发生唯一键冲突�?
...
SQLite �?`instant` SHOULD �?ISO 8601 UTC 字符串或 epoch milliseconds。团队必须统一一种表达，不得混用�?
...
| 租户状态列�?| `(tenant_id, organization_id, status, updated_at)` |
...
| 状态轮�?| `(status, created_at)` �?`(status, expire_at)` |
| 回调幂等 | `UNIQUE (provider, external_event_id)` |
| 消费者去�?| `UNIQUE (source_system, message_id, consumer_name)` |
| 树形子节�?| `(tenant_id, organization_id, parent_id, status, sort_order)` |
| 软删除列�?| `(tenant_id, status, deleted_at)` 或过滤索�?|
...
- 唯一约束 MUST 包含正确业务边界。例如租户内唯一应包�?`tenant_id`�?
- 高频更新字段不应放入过多索引，避免写放大�?
- 分页列表 SHOULD 使用稳定排序字段，例�?`(updated_at, id)` �?`(created_at, id)`�?
- 大表索引创建 MUST 使用在线创建能力或灰度窗口，避免长时间锁表�?

### 10.3 JSON、全文、空间索�?

- PostgreSQL JSON 高频查询 MAY 使用 GIN，但核心过滤字段�?SHOULD 拆列�?
- MySQL JSON 高频查询 SHOULD 使用生成列加索引�?
- SQLite 搜索场景 MAY 使用 FTS5 或投影列�?
- 复杂搜索 SHOULD 同步�?Elasticsearch、OpenSearch、Meilisearch、Typesense 或数据库全文索引�?
...
| 场景 | 推荐策略 |
| --- | --- |
| 按租户隔离的大表 | `tenant_id` 参与分片键或分区�?|
| 时间序列日志 | �?`created_at` �?�?日分�?|
| 账本流水 | 按租�?+ 时间或账�?+ 时间设计 |
| Webhook/任务记录 | �?`created_at` 分区，并�?TTL/归档 |
| 平台共享配置 | 不分片或�?`code` hash |

规则�?

- 分片�?MUST 出现在高频查询条件中�?
- 分片策略不能破坏唯一约束语义。全局唯一键需要全局 ID、全局索引、号段或协调服务�?
- 分区裁剪依赖的字�?MUST 在查询中显式出现�?
...
- 高频写入�?SHOULD 设置索引预算，例如核心写入路径索引不超过 5 个，超出必须说明收益�?
- 每个索引 MUST 绑定至少一个查询契约或约束目的�?
- 重复前缀索引 SHOULD 合并。例如已�?`(tenant_id, user_id, status, updated_at)` 时，通常不再需�?`(tenant_id, user_id)`，除非查询计划证明需要�?
- 大字段、低选择性字段、频繁变化字�?SHOULD 避免单独索引�?
- 容量超过百万级行的表 SHOULD 在设计期说明归档、分区或冷热策略�?
- 上线�?SHOULD 定期清理未使用索引，但删除索引也必须走结构变更评审�?
...
- 每张持久业务�?MUST 有主键�?
- 核心业务�?SHOULD 使用单列 `id` 主键，便�?SDK、消息、审计、读模型统一处理�?
- 极简关联�?MAY 使用复合主键，但一旦需要审计、状态、扩展字段、软删除或同步，SHOULD 改为�?`id` 的关系实体表�?
...
| 场景 | 建议 |
| --- | --- |
| 同库核心强一致关�?| SHOULD 建数据库外键 |
| 配置字典关系 | MAY 使用 code 引用 |
| 高频日志、流水、事�?| MAY 不建外键，但 MUST 建引用索�?|
| 跨服务、跨库、跨租户引用 | MUST NOT 建数据库外键，使用事件、补偿和校验任务 |
| 历史归档�?| SHOULD 谨慎建外键，避免归档受阻 |

外键�?MUST 有索引。跨租户 join MUST 校验租户边界，不能只�?`id` 关联�?

### 11.4 检查约�?

检查约�?SHOULD 用于不依赖业务上下文的规则：
...
表结构设计必须说明关键写入的并发控制方式。不同语言�?ORM 的默认事务行为不同，不能把并发正确性寄托在框架默认值上�?
...
| 唯一资源创建 | 数据库唯一约束作为最终防�?|
| 任务抢占 | `status` 条件更新、skip locked 或租约字�?|
| 跨服务状态推�?| outbox 事件 + 消费端幂�?|

规则�?

- 更新核心业务�?SHOULD �?`version` 条件：`WHERE id = :id AND version = :version`�?
- 金额、库存、配额、次数限�?MUST 使用数据库条件更新、锁、唯一约束或串行化队列保证正确性�?
...
| 架构 | 风险 | 标准要求 |
| --- | --- | --- |
| 主从复制 | replica lag 导致读旧数据 | 写后读路径必须可读主库或携带一致性策�?|
| 多区域只读副�?| 跨区域延�?| API 必须声明可接受延�?|
| 多主写入 | 冲突和唯一键竞�?| 必须有冲突解决策略和全局 ID |
| 分片数据�?| 跨分片唯一和事务困�?| 唯一键、事务边界、查询路由必须明�?|
| 边缘写入 | 离线合并冲突 | 必须有版本向量、事件合并或人工冲突处理 |

规则�?

- L3 �?MUST 声明读一致性要求：strong、read-your-writes、bounded-staleness �?eventual�?
- 写后立即查询�?API MUST 避免读到落后副本�?
- 唯一约束在多区域写入下必须有全局协调策略，不能只依赖单区域局部唯一�?
- 事件顺序不能简单依赖创建时间；跨区域事�?SHOULD 使用单调版本、序列号或聚合内版本�?
...
- 对外 API 稳定契约，例�?`status = "ACTIVE"`�?
- 人工读库排障和数据分析频繁依赖直观值�?

规则�?

- 字符串枚�?MUST 使用大写蛇形：`PENDING_REVIEW`�?
...
| 类型 | 示例 | 管理方式 |
| --- | --- | --- |
| reference data | 国家、币种、行业、地区、系统内置权�?| 版本化发布或受控配置发布 |
| seed data | 初始管理员角色、默认套餐、内置模�?| 可重复执行的幂等脚本 |
| lookup table | 状态映射、外部渠�?code、税率表 | �?owner、版本、生效时�?|
| feature config | 灰度开关、实验配�?| 配置中心或配置表，带审计 |

规则�?

- 种子脚本 MUST 幂等，重复执行不能产生重复数据�?
- 参考数�?SHOULD �?`code`、`name`、`status`、`effective_from`、`effective_to`、`version`�?
- 生产参考数据变�?MUST 有审计、审批或配置发布记录�?
- 不同环境的参考数据必须可比较，避免开发、测试、生�?code 不一致�?
...
- 金额 MUST NOT 使用 `FLOAT` �?`DOUBLE`�?
- 多币种场�?MUST �?`currency`�?
- 金融、支付、余额、账本场景中 decimal �?JSON �?MUST 使用字符串�?
- 金额精度必须由业务域定义。支付通常 2 位，小额计费�?4 �?8 位，链上资产或模型计费可更高�?
...
- 法币币种 SHOULD 使用 ISO 4217 三位代码，例�?`USD`、`CNY`、`EUR`�?
- 非法币资产、积分、token MUST 有资产字典，定义精度、展示单位、最小变更单位�?
- 涉及计费、税费、折扣、汇率的�?MUST 声明 rounding mode，例�?HALF_UP、HALF_EVEN、DOWN�?
- 税前金额、税额、税后金�?SHOULD 拆列，例�?`subtotal_amount`、`tax_amount`、`total_amount`�?
- 汇率必须记录来源、基准币种、报价时间和精度�?
- 不能在不同服务中分别计算最终金额。最终入账金额必须由一个事实来源服务生成，并写入账本或订单事实表�?

### 14.3 积分、余额、token、库�?

- 积分、点数、token �?SHOULD 使用整数�?
- 高并发余�?MUST 使用流水表和余额表，余额表可作为当前状态，流水表作为事实来源�?
...
- API MUST 返回 ISO 8601，例�?`2026-04-26T08:30:00Z`�?
- 不允许在同一字段中混用本地时区、UTC、epoch 秒、epoch 毫秒�?
- 定时任务、过期时间、账�?MUST 明确时间单位和时区�?
- 日期 `date` 与时�?`instant` MUST 区分。生日、账期日、自然日使用 `date`；创建时间、过期时间使�?`instant`�?
...
所有服务、任务、消息消费者、批处理�?MUST 维护等价上下文：
...
平台级查�?MUST 显式声明权限，不允许因为上下文缺失而默认全表查询�?
...
核心要求不是使用哪种框架，而是：租户过�?MUST 是基础设施能力，并且有测试证明普通查询无法绕过�?
...
权限过滤不应只依赖角色名。复杂系�?SHOULD �?RBAC、ABAC 和数据策略分层表达�?

| 模型 | 适用场景 | 数据库要�?|
| --- | --- | --- |
| RBAC | 后台菜单、角色权限、管理操�?| 角色、权限、关系表有唯一约束和租户边�?|
| ABAC | 按部门、地区、数据等级、资源属性授�?| 资源表有可过滤属性，策略表有版本 |
| RLS | 数据库原生行级安�?| 策略必须能从契约生成或审�?|
...
- 高敏数据�?MUST 记录访问审计策略，不只是写入审计�?
- 数据�?RLS 可以作为强防线，但应用层仍必须传递清晰的访问上下文�?
- ABAC 策略引用的字�?MUST 是独立列并建立必要索引，不能藏在 JSON 中�?
...
分布式系统不能仅靠“代码里先写库再发消息”保证可靠事件。涉及跨服务同步�?SHOULD 使用 outbox/inbox�?
...
- Inbox MUST 以消�?ID 和消费者名去重�?
- “Exactly once�?应理解为业务幂等后的效果一次，不应依赖消息中间件承诺�?
- 事件 payload MUST 有版本字段，消费�?MUST 兼容未知字段�?

## 18. 日志、审计和账本

### 18.1 不可变记�?

以下�?SHOULD 采用 append-only�?
...
append-only 表允许补充处理状态，�?MUST NOT 覆盖事实字段，例如原始金额、原�?payload、原始操作者�?
...
| 分类 | 示例 | 存储策略 |
| --- | --- | --- |
| 凭证 | 密码、API key、access token、refresh token、私�?| 不可逆哈希、加密、摘要或密钥管理系统 |
| 高敏 PII | 身份证号、银行卡号、手机号、邮箱、地址 | 加密、脱敏、盲索引、访问审�?|
| 业务敏感 | 价格策略、风控结果、内部备�?| 权限控制、审计、必要时加密 |
| 公开信息 | 昵称、公开描述、公开商品�?| 常规存储 |
...
- 字段契约 MUST 标注敏感等级，便�?SDK、日志、数据导出自动脱敏�?

### 19.2 加密和密钥轮�?

- 加密字段 SHOULD 记录 `key_id` 或密钥版本�?
- 密钥轮换 MUST 有双读或后台重加密策略�?
- 加密后仍需搜索的字�?MUST 设计盲索引或搜索代理，不能依赖明�?LIKE�?
- 加密字段排序和范围查询能力受限，设计前必须确认业务查询模式�?

### 19.3 数据留存

- 表契�?SHOULD 明确留存期限�?
- 可删除数�?MUST 定义删除方式：软删除、匿名化、物理删除、归档�?
- 法务冻结字段 `legal_hold` 一旦为 true，清理任�?MUST 跳过�?
...
| 日志输出 | 默认脱敏，禁止完�?token/key/password |
| 数据导出 | 导出任务记录申请人、范围、字段、审批、过期时�?|
| SDK | 敏感字段默认不生成写�?读取快捷方法，或标注高风�?|
| 数仓同步 | 分层脱敏，生产明细和分析宽表权限分离 |

规则�?

- 字段契约 MUST 标注 `sensitivity`，例�?`PUBLIC`、`INTERNAL`、`CONFIDENTIAL`、`SECRET`�?
- 高敏字段 SHOULD 标注 `masking_rule`，例如手机号 `138****8000`、邮箱局部脱敏、证件号尾号可见�?
- 数据出口 MUST 有审计记录，包括导出人、时间、查询条件、字段列表、文件位置、过期时间�?
- 不同环境的数据策略必须明确。测试、开发、演示环�?SHOULD 使用脱敏数据，不得直接复制生产高敏明文�?
...
如果前端�?SDK 选择 `camelCase`，转换必须在 SDK �?API adapter 层完成，数据库契约仍保持 `snake_case`�?

### 20.2 序列化规�?

| 逻辑类型 | JSON 表达 | 原因 |
| --- | --- | --- |
| `int64` | 字符�?| 避免 JavaScript number 精度丢失 |
| `decimal` | 字符�?| 避免浮点误差 |
| `instant` | ISO 8601 UTC 字符�?| 跨语言、跨时区一�?|
| `date` | `YYYY-MM-DD` | 不带时区 |
| `bytes` | base64 字符�?| JSON 兼容 |
| `enum_int32` | number + 文档映射 | 内部紧凑 |
| `enum_string` | 大写蛇形字符�?| 对外可读 |

`int64` 的字符串规则只适用�?HTTP JSON、浏览器、TypeScript SDK、GraphQL JSON gateway 等会经过 JavaScript number 的边界。数据库列、Rust 领域模型、SQL bind 参数、Java/Kotlin 领域模型、Go/C# 后端模型仍必须保持原生数值类型，例如 SQL `BIGINT`、Rust `i64`、Java `long`、Go `int64`、C# `long`。不得为了前�?JSON 安全而把 Rust 内部 ID、版本、序列号、计数器或金额最小单位改成字符串�?

浏览器提�?`int64` 时也必须提交字符串；服务�?HTTP adapter 在请求边界负责解析、校验正负号和范围，再把原生数值传给领域逻辑和数据库。OpenAPI 中面向浏览器�?`int64` 必须声明�?`type: string`、`format: int64`、数�?pattern，并�?`x-sdkwork-int64-string: true` 和实现侧原始类型扩展标明边界转换�?

### 20.3 版本和并�?

- 更新接口 SHOULD 携带 `version` �?ETag，避免丢失更新�?
- 批量接口 MUST 明确部分成功策略�?
- 分页 SHOULD 使用游标或稳定排序。大表不建议只用 offset。
- 列表查询 MUST 在数据库或增量维护的有序索引上完成分页；严禁先全量加载到应用进程再 `skip`/`take`/`slice` 分页。详见 `PAGINATION_SPEC.md`。
...
- `int64` 字段 SHOULD 声明�?`type: string`，并通过 `format` 或扩展标注原始逻辑类型�?
- decimal 字段 SHOULD 声明为字符串，并说明精度�?
- 时间字段 MUST 声明�?ISO 8601 UTC�?
- 敏感字段 SHOULD 使用扩展标记，例�?`x-sensitive: true`�?

GraphQL�?

- `int64` �?decimal SHOULD 使用自定�?scalar，例�?`LongString`、`DecimalString`�?
- 不要使用 GraphQL `Int` 表示 64 位整数，因为 GraphQL `Int` �?32 位有符号整数�?
- 时间 SHOULD 使用 `DateTime` scalar，并强制 UTC�?

gRPC/Protobuf�?

- 内部 gRPC MAY 使用 `int64`，但任何会转 JSON �?gateway MUST 按字符串策略处理�?
- decimal SHOULD 用字符串�?`{ units, nanos }` 等明确结构，不能�?double�?
- 时间 SHOULD 使用 `google.protobuf.Timestamp`，并在边界层转为 ISO 8601 UTC�?

### 20.5 查询、排序和分页契约

数据库索引必须服务明确的查询契约。任何对外列�?API 或跨服务查询接口 SHOULD 先定义查询形态，再设计索引�?
...
| 最�?page size | 防止无限制查�?|
| 租户边界 | 查询是否必须�?`tenant_id`、`organization_id`、`user_id` |
| 一致�?| 是否允许读延迟、是否读主库 |

规则�?

- 大表列表 SHOULD 使用 seek/cursor pagination，例�?`(updated_at, id)` 游标�?
- offset pagination 只适合小表、后台低频查询或已限制最大页数的场景�?
- 排序字段必须稳定；只�?`updated_at` 排序可能产生重复或漏读，SHOULD 增加 `id` 作为 tie-breaker�?
- 对外 API MUST 限制 `page_size` 上限�?
- 不允许把任意字段排序、任意字段过滤暴露给前端，除非有查询网关、索引保护和成本控制�?
- 查询契约变化会影响索引和 SDK，必须进�?schema review�?
- 持久化表的列表查询 MUST 在 SQL 层使用 `LIMIT` 或 keyset 谓词完成分页；Repository `find_all` 后在 Service 层 `skip`/`take` 属于禁止的进程内分页反模式。
- 内存投影/运行时库存的列表 MUST 在写入时维护有序索引，并在索引迭代器上做有界窗口；禁止每次请求重建无界集合并再切片。
- 跨层分页实现、SDK/前端消费与验收规则以 `PAGINATION_SPEC.md` 为准。
...
| `updated_at` | 创建和更新时�?UTC 当前时间 |
| `version` | 创建�?0，更新时递增 |
| `tenant_id` | 从认证、任务或消息上下文解�?|
| `organization_id` | 从上下文或业务归属解�?|
...
Rust 中金�?MUST NOT 使用 `f64`。推�?`rust_decimal::Decimal` 或整数最小单位�?
...
Go �?JSON 对外 SHOULD �?`int64` 使用 `,string`�?
...
PHP �?32 位环境中不能安全承载 64 位整数。跨环境 SDK SHOULD �?`int64` 当作字符串处理。金�?SHOULD 使用字符串或 decimal 扩展，不使用 float�?

### 21.7 Java/Kotlin

Java/Kotlin 可以使用 JPA、MyBatis、jOOQ、JDBC、Exposed、Quarkus Panache 或手�?SQL。标准只要求字段语义一致，不要求依赖某�?ORM。`Instant`、`Long`、`BigDecimal` SHOULD 分别映射 `instant`、`int64`、`decimal`�?

## 22. 结构演进规范

### 22.1 契约版本

数据库契�?SHOULD 使用语义化版本或等价版本策略�?

| 版本变化 | 含义 | 示例 |
| --- | --- | --- |
| PATCH | 不影响应用语义的兼容修复 | 增加注释、增加非唯一索引 |
| MINOR | 向后兼容能力增加 | 新增可空字段、新增枚举值、JSON 新增可选字�?|
| MAJOR | 破坏性变�?| 删除字段、字段改义、唯一键边界变化、decimal 精度降低 |
...
- 时间统一�?ISO 8601 �?epoch milliseconds�?
- decimal 存字符串或最小单位整数�?
- JSON �?`TEXT` 并由应用校验�?
...
- 删除字段�?MUST 确认 CDC consumer、数仓、报表、搜索索引、缓存、SDK 生成器均已切换�?
- 字段重命�?SHOULD 视为“新增新字段 + 双写 + 下游切换 + 删除旧字段”，不要直接 rename�?
- 枚举新增�?MUST 提前通知下游，消费者必须能处理未知值�?
- JSON payload 新增字段必须保持向后兼容；删除或改义属于破坏性变更�?
- CDC 事件 SHOULD �?schema version 或结构变更版本，便于消费者灰度处理�?
- 回填任务 SHOULD 标记来源，避免触发误告警或重复业务动作�?

### 22.9 回滚和前�?

数据库变更不总能安全回滚。每个结构变�?MUST 明确采用回滚还是前滚�?

| 类型 | 策略 |
| --- | --- |
| 新增�?字段 | 通常可回滚，但需确认应用版本 |
| 新增索引 | 可回滚，注意耗时 |
| 数据回填 | 通常采用前滚修复，回滚前需备份旧�?|
| 字段删除 | 不允许直接回滚，必须依赖备份或重新回�?|
| 类型收缩 | 高风险，必须先验证无截断 |
| 唯一约束变更 | 需提前清理冲突数据 |

发布顺序 SHOULD 先部署兼容应用，再执行扩展变更，再切换读写，最后收缩�?

### 22.10 Schema Drift 和环境一致�?
...
- 生产环境 SHOULD 定期导出 schema，与契约�?drift 检测�?
- 紧急手工变更必须在 24 小时内补齐结构变更脚本和契约记录�?
- 不允许只�?ORM 模型而不提交结构变更记录，也不允许只改数据库不更�?API/SDK 契约�?
- 测试环境、预发环境、生产环境的参考数据和 schema 版本 SHOULD 可自动比对�?
- 漂移修复必须优先保护数据，不得直�?drop 未知字段，除非确认无消费者和无数据价值�?

## 23. 数据生命周期

### 23.1 状态机

核心业务�?SHOULD 定义状态机，而不是只定义状态枚举。状态机文档至少包含�?
...
临时表、导入表、任务中间表、幂等记�?MAY 使用 TTL 清理�?

- 必须�?`expire_at` 或等价字段�?
- 清理任务必须限速�?
- 清理任务必须跳过 `legal_hold = true`�?
- 重要数据清理�?SHOULD 有审计或归档�?
...
| R1 | 可从上游重放恢复，例�?outbox 派生数据 |
| R2 | 需要常规备份恢复，例如普通业务表 |
| R3 | 关键数据，需�?PITR、异地备份、恢复演练，例如账本、支付、审�?|

规则�?

- 关键业务�?MUST 有恢复点目标 RPO 和恢复时间目�?RTO�?
- 结构变更�?SHOULD 评估是否需要快照备份�?
...
- 投影字段缺失或延�?MUST �?API 语义中说明�?

### 24.1 数据质量规则

核心�?SHOULD 定义数据质量规则，供 CI、结构演进校验、离线巡检和告警使用�?

| 规则类型 | 示例 |
| --- | --- |
| 完整�?| `uuid` 非空、`tenant_id` 非空、金额非�?|
| 唯一�?| `business_no` 无重复、幂等键无重�?|
| 合法�?| 枚举值在允许集合内、金额非�?|
| 一致�?| 流水汇总等于余额、订单支付状态一�?|
| 时序�?| `created_at <= updated_at`、`started_at <= finished_at` |
| 引用�?| 引用对象存在，跨服务引用可通过 API 或快照校�?|
...
L3 �?SHOULD 有仪表盘和告警阈值。结构变更窗口内 SHOULD 临时提高监控粒度�?
...
- 进入数仓、湖仓、搜索、缓存、消�?topic 的字�?SHOULD �?lineage 记录�?
- 字段删除或改义前 MUST 查询血缘，确认下游消费者已经切换�?
- 手工报表、临时脚本、外�?BI 如果依赖生产表，也应登记为消费者�?
- 数据目录 SHOULD �?schema registry 共享字段 ID 或稳定路径，避免字段重命名后血缘断裂�?
- L3 表的关键字段 SHOULD 标注数据新鲜�?SLO，例�?CDC 延迟小于 5 分钟�?
...
| 存储类型 | 适配方式 |
| --- | --- |
| 文档�?| 文档必须包含 `id/uuid/created_at/updated_at/version` 等等价字段，核心查询字段仍需独立索引 |
| 键值库 | key 命名必须包含租户和业务边界，value schema 必须版本�?|
| 搜索引擎 | 索引是读模型，必须有来源对象和同步版�?|
| 列式�?| 作为日志、分析、审计派生库，不替代强事务主�?|
| 图数据库 | 节点和边仍需稳定 ID、类型、版本、审计字�?|
| 对象存储 | 元数据表必须记录对象 key、hash、大小、MIME、owner、生命周�?|
| 湖仓/数仓 | 表必须保留来源系统、同步批次、schema version 和数据日�?|
| 流式平台 | 消息 key、事件版本、幂等键和消费者去重策略必须明�?|
...
- Kafka、Pulsar、RabbitMQ 等事件系统中的消�?schema 必须版本化，并能映射回数据库契约�?
- SDKWork 自有文件、上传会话、下载授权、对象存�?provider、bucket/object 生命周期 MUST �?`DRIVE_SPEC.md` 定义�?Drive 表族管理；业务域不得自建对象 blob 表、上传会话表、provider 注册表或 presign 生命周期�?
- S3、OSS、MinIO 等对象存储中的文件必须通过 Drive 元数据表�?manifest 管理，不允许只把对象 key 散落在业�?JSON 中�?
- 图片、视频、音频、语音、文档、归档文件、AI 生成媒体、商品素材等媒体资源 MUST 遵循 `MEDIA_RESOURCE_SPEC.md`，并�?SDKWork 自有存储场景中使�?Drive 作为存储权威。业务表不得把临时签�?URL、CDN 派生 URL 或裸对象 key 当作事实来源�?
- 业务域需要引用媒体时 SHOULD 使用 `drive_space_id`、`drive_node_id`、`drive_uri`、`media_resource_id`、Drive 暴露�?`object_blob_id` 或领域关系表，并通过 `media_role` 表达头像、封面、缩略图、主图、详情图、SKU 图、视频、附件、发票文档等业务角色�?
- `media_resource_snapshot` 这类 JSON 字段只允许作为读模型投影，必须有 `MediaResource` 结构契约，并且不能替代稳定资源引用�?
- 新应用的媒体字段 MUST 直接使用稳定媒体引用或领域媒体关系表；`*_url`、`cover_image`、`thumbnail_url`、`video_url`、`asset_url` 等裸媒体 URL 字段不得作为标准 schema 进入数据库契约�?
...
| DB002 | MUST | 持久业务表有主键 |
| DB003 | MUST | L1 及以上业务表�?`id`、`created_at`、`updated_at` |
| DB004 | SHOULD | 核心业务表有 `uuid` 唯一约束 |
| DB005 | MUST | 多租户表�?`tenant_id` |
| DB006 | MUST | 多租户列表索引以 `tenant_id` 开�?|
| DB007 | MUST | 金额字段不用 float/double |
| DB008 | MUST | `int64` API 序列化策略已声明 |
| DB009 | SHOULD | 状态字段有枚举文档或字典表 |
| DB010 | MUST | 幂等场景有唯一�?|
| DB011 | SHOULD | JSON 字段�?schema �?DTO |
| DB012 | MUST | 敏感字段有分类和存储策略 |
| DB013 | MUST | 破坏性结构变更有扩展收缩计划 |
| DB014 | SHOULD | 外键列有索引 |
| DB015 | SHOULD | 大表新增索引说明在线策略 |
| DB016 | MUST | 时间字段统一 UTC/ISO 8601 策略 |
| DB017 | SHOULD | 软删除表唯一键策略明�?|
| DB018 | MUST | L3 表有审计、留存、回滚和校验方案 |
| DB019 | SHOULD | 派生表有来源对象和同步版�?|
| DB020 | MUST | 平台级跨租户查询显式授权 |
| DB021 | MUST | 共享表有事实来源和写入所有�?|
| DB022 | SHOULD | 契约有版本号和兼容窗�?|
| DB023 | MUST | 大小写不敏感唯一字段有规范化策略 |
| DB024 | SHOULD | 大表有分区、分片或增长治理说明 |
| DB025 | MUST | 关键并发写入有锁、版本、条件更新或唯一约束 |
| DB026 | SHOULD | 列表查询有排序和分页契约 |
| DB027 | MUST | 对外 API 限制最�?page size |
| DB027A | MUST | 持久化列表在 SQL/keyset 或增量索引上分页；禁止进程内全量加载后切片（见 PAGINATION_SPEC.md） |
| DB028 | SHOULD | L3 表有 RPO/RTO 和恢复演练证�?|
| DB029 | SHOULD | CDC 下游兼容进入结构演进计划 |
| DB030 | MUST | 字段删除前确�?SDK、数仓、搜索、缓存消费者已切换 |
| DB031 | SHOULD | 数据质量规则覆盖完整性、唯一性、合法性、一致�?|
| DB032 | SHOULD | 关键表有慢查询、锁等待、CDC 延迟和数据质量监�?|
| DB033 | MUST | 密码、token、私钥不出现在日志、审计、错误表明文字段�?|
| DB034 | SHOULD | 对象存储资源有元数据表或 manifest |
| DB035 | MUST | 事件 schema 有事件版本和消费者未知字段兼容策�?|
| DB036 | SHOULD | 文本唯一字段声明字符集、排序规则和规范化方�?|
| DB037 | MUST | 核心表声明业务域和写�?owner |
| DB038 | SHOULD | 跨服务共享语义声�?bounded context |
| DB039 | SHOULD | 快照和反规范化字段声明来源和 schema version |
| DB040 | MUST | EAV/JSON 不承载金额、状态、租户、权限、幂等等核心字段 |
| DB041 | SHOULD | L2/L3 表有容量模型和索引预�?|
| DB042 | SHOULD | 重复或无查询契约绑定的索引进入清理清�?|
| DB043 | MUST | 高敏字段声明 sensitivity �?masking_rule |
...
| DB050 | SHOULD | 对外公开 ID 不直接使用连续自增主�?|
| DB051 | MUST | 第三�?ID 使用 provider + external_id 唯一边界 |
| DB052 | MUST | 金额字段声明币种、精度和 rounding mode |
| DB053 | SHOULD | 税费、折扣、汇率字段声明计算口径和事实来源 |
| DB054 | MUST | L3 表声明读一致性要求和副本延迟策略 |
| DB055 | MUST | 数据驻留和跨境同步要求进入安全评�?|
| DB056 | SHOULD | reference/seed/lookup 数据由幂等脚本或受控发布管理 |
| DB057 | MUST | 对外发布�?reference code 不得复用为新含义 |
| DB058 | SHOULD | CI 或巡检执行 schema drift 检�?|
| DB059 | MUST | ORM、DDL、schema registry、API/SDK 契约变更保持同步 |
| DB060 | SHOULD | L3 表有运维 runbook 和演练记�?|
| DB061 | MUST | 新建业务表第一段为受控业务模块前缀 |
| DB062 | MUST | 表名前缀不得使用产品名、项目名、公司名或技术栈名作为默认业务前缀 |
| DB063 | SHOULD | 产品/部署命名空间放在 schema/catalog/database 层，而不是表名第一�?|
| DB064 | MUST | 模块前缀必须登记 owner、bounded context 和示例表 |
| DB065 | MUST | 跨模块共享表使用事实来源模块前缀 |
| DB066 | SHOULD | 存量项目级前缀表有目标模块前缀映射和标准化整改说明 |
| DB067 | MUST | 同一物理表名被多个实�?契约映射时有共享语义说明或冲突整改计�?|
| DB068 | MUST | 存量表名前缀合规差距标注 P0/P1/P2/P3 优先级和风险原因 |
| DB069 | MUST | `owner-review` 表完成事实来�?owner 确认后才能进入标准目标名确认 |
| DB070 | SHOULD | 外部渠道、连接器、供应商账号表使�?`integration_` 前缀 |
| DB071 | SHOULD | 工作空间、项目、应用、模板等设计时资产使�?`studio_` 或更具体业务前缀 |
| DB072 | SHOULD | 实体注解、DDL、结构变更脚本、审计文件和 schema linter 规则保持同一前缀注册�?|

检查输�?SHOULD 标注表名、字段名、规则编码、严重级别、修复建议�?
...
- [ ] 已定�?`id` 生成策略�?
- [ ] 已定�?`uuid` 或等价外部稳�?ID�?
- [ ] 已包�?`created_at/updated_at/version`，或说明为什么不需要�?
- [ ] 已明确是否需�?`tenant_id/organization_id/data_scope`�?
- [ ] 已明确是否需�?`user_id`、`owner_type/owner_id`�?
- [ ] 已定�?`status` 和状态机�?
- [ ] 已定义软删除、归档、物理删除策略�?
- [ ] 已定义唯一键和业务边界�?
- [ ] 已定义高频查询和对应索引�?
...
- [ ] 已给�?PostgreSQL/MySQL/SQLite 等目标数据库映射�?
- [ ] 已给�?TypeScript/Python/Rust/Go/Java/C#/PHP 等目标语言映射�?
- [ ] 已设计结构演进、回填、校验和回滚策略�?
...
建议�?100 分制记录数据库设计成熟度。低�?80 分的新核心表不应进入生产�?

| 类别 | 分�?| 检查重�?|
| --- | ---: | --- |
| 字段契约 | 15 | 标识、审计、命名、逻辑类型、空值策�?|
| 隔离和权�?| 15 | 租户、组织、用户、owner、平台共享数�?|
| 约束和索�?| 15 | 主键、唯一键、查询索引、外键或应用级校�?|
| 结构演进能力 | 15 | 扩展收缩、回填、校验、回�?前滚、CDC |
| 跨语言/API | 10 | int64、decimal、时间、枚举、SDK 映射 |
| 安全合规 | 10 | 敏感字段、加密、脱敏、审计、留�?|
| 可观测和幂等 | 10 | request_id、trace_id、idempotency_key、事件一致�?|
| 数据生命周期 | 10 | 状态机、软删除、归档、TTL、法务冻�?|
...
每张新核心表 SHOULD 附带一份表设计说明。可以是 Markdown、YAML、JSON �?schema registry 记录�?
...
评审�?SHOULD 优先审这份契约，再审物理 DDL �?ORM 代码�?
...
- 支付回调没有唯一事件约束�?
- 金额使用 float/double�?
- JavaScript 前端直接接收 64 位整�?number�?
- 同一时间字段混用本地时间�?UTC�?
- 在应用已经上线后直接删除字段�?
- 用字符串文案作为枚举值�?
- �?`metadata` 存所有核心业务字段�?
...
4. 从契约生�?DDL、ORM 模型、OpenAPI schema、SDK DTO 或反向校验�?
...
- 每个业务�?SHOULD �?schema owner�?
- 公共字段变更 MUST 评审�?
- 跨服务表或共享表 MUST 有契约版本�?
- SDK 生成器和数据库结构变更工�?SHOULD 读取同一份契约�?
...
- 开发、测试、预发、生�?SHOULD 使用同一套结构变更机制�?
- 测试数据 fixture SHOULD 版本化，并与参考数据版本匹配�?
- 测试环境不得依赖生产高敏明文数据。需要生产样本时必须脱敏、抽样和审批�?
- 预发环境 SHOULD 使用接近生产�?schema、索引、数据量和配置，避免小数据量掩盖慢查询�?
...
| Hibernate filter | 通用租户、组织、用户查询过滤规则的一种实�?|

既有服务可以继续使用原有基础类，但跨语言、跨服务、跨数据库的公共契约应输出本文的通用字段命名和语义�?

### 30.1 当前工程表名前缀兼容映射

�?`legacy-java-plus-entity/src/main/java/com/sdkwork/spring/ai/plus/entity` 的实体表定义进行扫描，用于校验本规范对既有系统的兼容映射能力，当前结论：

- 扫描 `278` �?Java 文件，识别到 `196` �?`@Entity + @Table(name=...)` 实体表定义�?
- 实体表唯一物理表名�?`193` 个�?
- 所有实体表名第一段均�?`plus`�?
- `src/main/resources/database` �?`7` �?SQL 结构变更文件包含 `56` �?`create table/alter table` 表引用，涉及 `23` 个唯一表名，第一段也全部�?`plus`�?
- `plus` 是产�?项目级命名空间，不是业务模块前缀�?
- 当前实体�?SQL 表名不满足新标准 `<module_prefix>_<entity_name>` �?L1 命名要求，只能视�?L0 legacy compatible�?
- 发现 3 组实体共享同一物理表名，需要单独评审是否为有意复用还是命名冲突�?
- 全量实体映射、目标前缀、合规优先级和风险标记已固化�?`DATABASE_TABLE_PREFIX_AUDIT.md`，用于标准化评审和工具校验�?
- 本模块提�?`scripts/check-table-prefix.ps1` 作为本地巡检入口；默�?report 模式用于观察 legacy 状态，`-Strict` 模式可接�?CI 阻断新非标准表名�?
...
| 当前包模�?| 目标业务前缀 | 说明 |
| --- | --- | --- |
| `agent`, `model`, `prompt`, `tool`, `generation`, `character`, `skill`, `plugin`, `usage` | `ai` | AI 能力�?|
| `record` 中的调用/用量事实 | `ai` | AI 调用记录、调用明细、用量事�?|
| `user`, `tenant`, `organization`, `rbac`, `security`, `invitation` | `iam` | 身份、租户、权限、安�?|
| `platform` 中的 API key、安全策�?| `iam` | 访问凭证�?API 安全策略 |
| `platform` 中的 channel/account/proxy/resource | `integration` | 外部渠道、供应商连接器和代理配置 |
| `app`, `project`, `workspace`, `docs`, `ppt` | `studio` | 设计时工作空间、项目、应用、文档页、模�?|
| `trade`, `account`, `currency`, `coupon`, `product`, `vip`, `card`, `invoice`, `shop`, `partner` | `commerce` | 交易、账户、商品、会员、佣金伙�?|
| `article`, `comments`, `collection`, `favorite`, `feeds`, `news`, `tags`, `share`, `detail`, `category`, `vote` | `content` | 内容和互�?|
| `files`, `image`, `video`, `voice`, `music`, `human`，以及媒体发布记�?| `media` | 文件和媒体资�?|
| `conversation`, `message`, `topic`, `im`, `rtc`, `sns` | `comms` | 会话、消息、实时通信、社交关�?|
...
| 优先�?| 当前数量 | 判定标准 | 处理要求 |
| --- | ---: | --- | --- |
| P0 | 6 | 重复物理表名或目标表名冲�?| 先解决实体映射冲突，禁止把冲突带入标准命�?|
| P1 | 38 | 核心事实表、跨模块表、owner 需要复核的�?| 完成 owner-review 和事实来源确�?|
...
- `owner-review` 表必须先确认事实来源模块，例�?`plus_app`、`plus_project`、`plus_workspace` 归入 `studio_` 后，引用它们的字段注释、外键、索引名也要同步更新为目标语义�?
...
- `metadata` �?schema�?
- `int64` �?API 中作为字符串�?
- 删除是软删除，使�?`deleted_at/deleted_by`�?
- 更新使用 `version` 防止并发覆盖�?
...
- 逻辑类型映射�?PostgreSQL、MySQL、SQLite、SQL Server、Oracle、ClickHouse 等数据库可以互相移植、同步或替换�?
...
所�?SDKWork 应用 MUST 使用统一的数据库连接池管理，禁止在业务代码中直接创建连接池�?

**核心要求�?*
- 所有数据库连接�?MUST 通过 `sdkwork-database` 统一创建和管�?
- 禁止在业务代码中直接使用 `SqlitePoolOptions::new()` �?`PgPoolOptions::new()`
- 测试代码可以使用简化的内存数据库创建方�?

### 33.2 sdkwork-database 架构

`sdkwork-database` �?SDKWork 统一的连接池管理库，包含两个 crate�?

| Crate | 用�?|
|-------|------|
| `sdkwork-database-config` | 配置类型定义，不依赖 sqlx |
| `sdkwork-database-sqlx` | sqlx 连接池实现，依赖 sdkwork-database-config |

### 33.3 配置标准

#### 环境变量命名

所有服�?MUST 使用统一的环境变量命名：

```
SDKWORK_{SERVICE}_DATABASE_URL           # 数据库连�?URL
SDKWORK_{SERVICE}_DATABASE_ENGINE        # 数据库引�?(sqlite/postgres)
...
**向后兼容�?* 优先使用 `SDKWORK_{SERVICE}_DATABASE_URL`，回退�?`DATABASE_URL`�?
...
| synchronous | Normal | WAL 模式下的最佳平�?|
...
所�?PostgreSQL 连接�?MUST 应用以下配置�?

| 配置�?| 推荐�?| 说明 |
|--------|--------|------|
| max_connections | 10-16 | 根据数据�?max_connections 调整 |
| min_connections | 1-2 | 保持最小连接数 |
| acquire_timeout | 10s | 等待连接的最大时�?|
| idle_timeout | 300s | 空闲连接关闭时间 |
| max_lifetime | 1800s | 连接最大生命周�?|
...
2. 替换连接池创建代码：
   ...
   // 之后
   let pool = sdkwork_database_sqlx::create_pool_from_env("MY_SERVICE")
       .await?
       .expect("SDKWORK_MY_SERVICE_DATABASE_URL not set");
   ```
...
sdkwork-database-repository �ṩͳһ�� Repository ģʽ�������� SQLite �� PostgreSQL �Ĳ��졣

**����Ҫ��**
- �������ݿ� CRUD ���� SHOULD ʹ�� Repository ģʽ
- ���Ӳ�ѯ����ʹ��ԭ�� SQL
- Entity �� Repository ͨ�����Զ�����
...
2. **ͨ���ֶ�**
   - ʹ�� AutoTimestamp �Զ����� created_at �� updated_at
   - ʹ�� SoftDelete ʵ����ɾ��
   - ʹ�� Versioned ʵ���ֹ���

### 35.2 Repository ʹ�ù淶

1. **CRUD ����**
   - ʹ�� impl_repository! ���Զ����� Repository
   - �� CRUD ʹ�� Repository ����
   - ���Ӳ�ѯʹ�� Query ��������ԭ�� SQL

2. **��ѯ�Ż�**
   - ʹ�� find_first() ��� find_all().first()
   - ʹ�� find_paginated() ���з�ҳ��ѯ
   - ʹ�� count() ������ݴ�����
...
| ���� | ��; | ʹ�ó��� |
|------|------|----------|
| AutoTimestamp | �Զ�ʱ��� | ������Ҫ��Ƶ�ʵ�� |
| SoftDelete | ��ɾ�� | ��Ҫ������ʷ���ݵ�ʵ�� |
| Versioned | �ֹ��� | �������³��� |
| Pagination | ��ҳ���� | �б��ѯ |
| PaginatedResponse | ��ҳ��Ӧ | API ���� |
| QueryFilter | ��ѯ���� | ���Ӳ�ѯ���� |
...
所有数据库服务 SHOULD 实现健康检查机制。

### 36.2 健康检查指标

| 指标 | 说明 | 健康阈值 |
|------|------|----------|
| latency_ms | 查询延迟 | < 1000ms |
| pool_size | 连接池大小 | > 0 |
| idle_connections | 空闲连接 | > 0 |

### 36.3 数据库迁移

所有数据库 schema 变更 SHOULD 使用迁移管理。

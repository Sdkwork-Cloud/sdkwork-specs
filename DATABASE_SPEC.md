...
| ID 类型 | 用途 | 推荐策略 |
| --- | --- | --- |
| `id` | 内部主键、join、审计引用 | 默认 64 位正整数雪花 ID，由应用/服务统一 ID Provider 生成 |
...
| 字段 | 标准值 | 说明 |
| --- | --- | --- |
| 符号位 | 1 bit 保留为 0 | ID MUST 为正 `int64`，避免跨语言有符号整数问题 |
| 时间戳 | 41 bit 毫秒 | 从 SDKWork 默认 epoch 起算，可使用约 69.7 年，满足 30 年以上不重复要求 |
| 节点 ID | 10 bit | 单个部署唯一节点号，范围 `0..1023` |
| 毫秒序列 | 12 bit | 单节点每毫秒 `4096` 个 ID |
| 默认 epoch | `2024-01-01T00:00:00Z` | Rust `sdkwork_platform_id_service::default_snowflake_epoch_millis()` 的公开值 |
| 最低寿命 | `30` 年 | 自定义 profile MUST 提供容量证明且不得低于该值 |
...
- ID 生成器 MUST 明确节点 ID、时钟回拨、序列溢出、重启恢复、容量证明、冲突检测和监控策略。
- 运行时业务数据 INSERT 的 `BIGINT id` MUST 由统一 ID Provider 显式生成并绑定写入；仓储、DAO、ORM repository、SQL mapper、后台任务、事件消费者和迁移后的写入路径不得各自临时实现取号逻辑。
- SDKWork Rust 实现 MUST 优先复用 `sdkwork-appbase/crates/sdkwork-platform-id-service` 中的 `sdkwork_platform_id_service::SnowflakeIdGenerator`；其他语言实现应在 appbase 对应包中提供等价能力，且遵守同一 profile。
- 写入框架 MUST 在进入数据库前准备 `id`、`uuid`、审计时间、租户/组织/用户上下文和幂等键；业务服务只能依赖该写入框架或注入的 ID Provider，不能在 SQL 片段中隐藏 ID 分配。
- 运行时业务表 DDL MUST 使用 `id BIGINT NOT NULL PRIMARY KEY` 或数据库等价的非自动分配 `int64` 主键；SQLite 不得使用 `INTEGER PRIMARY KEY` 作为运行时业务 ID，因为它会回落到 rowid 自动分配语义。
- 运行时业务数据 INSERT MUST 显式列出 `id` 列并绑定 ID Provider 生成的值。`RETURNING id` 只允许返回已经绑定的 ID，不得作为数据库分配 ID 的证据。
- 运行时业务数据 INSERT MUST NOT 依赖 `BIGSERIAL`、`SERIAL`、`AUTOINCREMENT`、`GENERATED ... AS IDENTITY`、`DEFAULT nextval(...)`、单条数据库 `RETURNING id`、`last_insert_rowid()`、`MAX(id)+1` 或本地哈希派生 ID 作为主键分配策略；历史表、临时测试表、外部 raw 落地表和已登记迁移窗口中的兼容表除外。
- 节点 ID MUST 由部署配置、编排平台、实例注册表或受控本地配置提供；生产环境不得在进程启动时随机选择节点 ID。测试环境可以使用保留节点区间，但不得泄漏到生产配置。
- 时钟回拨 MUST 失败关闭或在明确上限内等待后失败并告警；不得在观察到回拨后继续生成早于上一成功时间戳的 ID。
- 序列溢出 SHOULD 等待下一毫秒；如果运行时无法等待，MUST 失败关闭并记录可观测错误。
- 自定义 ID 策略 MAY 使用不同 bit 分配、号段或外部 ID 服务，但 MUST 在契约中声明 `type`、适用表/范围、容量和寿命证明、节点/号段冲突防护、时钟/溢出/重启策略、失败关闭行为、观测指标、跨语言序列化和回滚方案。未证明 30 年以上容量的自定义策略不得作为默认运行时业务 ID。
...
- SDKWork 自有文件、上传会话、下载授权、对象存储 provider、bucket/object 生命周期 MUST 由 `DRIVE_SPEC.md` 定义的 Drive 表族管理；业务域不得自建对象 blob 表、上传会话表、provider 注册表或 presign 生命周期。
- S3、OSS、MinIO 等对象存储中的文件必须通过 Drive 元数据表或 manifest 管理，不允许只把对象 key 散落在业务 JSON 中。
- 图片、视频、音频、语音、文档、归档文件、AI 生成媒体、商品素材等媒体资源 MUST 遵循 `MEDIA_RESOURCE_SPEC.md`，并在 SDKWork 自有存储场景中使用 Drive 作为存储权威。业务表不得把临时签名 URL、CDN 派生 URL 或裸对象 key 当作事实来源。
- 业务域需要引用媒体时 SHOULD 使用 `drive_space_id`、`drive_node_id`、`drive_uri`、`media_resource_id`、Drive 暴露的 `object_blob_id` 或领域关系表，并通过 `media_role` 表达头像、封面、缩略图、主图、详情图、SKU 图、视频、附件、发票文档等业务角色。

...
| 默认 epoch | `2024-01-01T00:00:00Z` | Rust `sdkwork_id::default_snowflake_epoch_millis()` 的公开值 |
...
- SDKWork Rust 实现 MUST 优先复用 `sdkwork-appbase/packages/native-rust/foundation/sdkwork-id-rust` 中的 `sdkwork_id::SnowflakeIdGenerator`；其他语言实现应在 appbase 对应包中提供等价能力，且遵守同一 profile。

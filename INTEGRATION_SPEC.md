...
- Appbase app SDK consumers `MUST` provide `Authorization: Bearer <JWT auth_token>` and `Access-Token: <JWT access_token>` through SDK/bootstrap auth APIs for protected app-api operations.
- Appbase backend SDK consumers `MUST` provide `Authorization: Bearer <JWT auth_token>` and `Access-Token: <JWT access_token>` through backend SDK/bootstrap auth APIs for protected backend-api operations.
- Open-api consumers `MUST` use the credential mode declared by the open-api contract (`api-key`, `oauth`, or `open-api-flexible`). The server resolves API key context through the appbase API key parser and lookup-service framework, and OAuth bearer context through the OAuth token lookup framework.

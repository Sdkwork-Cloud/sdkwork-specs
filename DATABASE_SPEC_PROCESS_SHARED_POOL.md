...
- Runtime profile loaders and database environment resolvers `MUST` preserve `SDKWORK_DATABASE_TEMPORARY_*` process-governance keys while replacing database identity or credential keys. Dropping these controls during env materialization makes the declared exception non-executable and is a startup-contract violation.

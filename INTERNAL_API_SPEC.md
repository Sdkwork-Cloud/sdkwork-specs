...
- SDK generation `MUST` follow `SDK_SPEC.md` with `--standard-profile sdkwork-v3` when the generator profile supports the internal prefix; otherwise the SDK family `MUST` document the supported generator mode in `.sdkwork-assembly.json` and `sdk-manifest.json`.

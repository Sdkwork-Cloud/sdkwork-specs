...
| Route and URL ownership | `API_SPEC.md`, `APPLICATION_GATEWAY_SPEC.md`, `HEALTH_CHECK_SPEC.md` | Route manifest owner/surface/path, OpenAPI `x-sdkwork-owner`, `x-sdkwork-api-authority`, `x-sdkwork-api-surface` | `check-route-path-collisions.mjs`, gateway assembly validation, OpenAPI materialization tests |

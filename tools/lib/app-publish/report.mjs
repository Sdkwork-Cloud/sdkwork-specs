/**
 * App publish run report. Emitted as JSON so it can be archived as release
 * evidence per RELEASE_SPEC.md §4 (application release type).
 *
 * Mirrors lib/sdk-publish/report.mjs shape, adding app-specific fields
 * (architecture, platform, artifactName, checksum).
 */
...
export class ReportBuilder {
  ...
  printConsole() {
    ...
    console.log('');
    console.log(`app publish run ${this.runId} (${this.mode})`);
    ...
    if (failures.length > 0) {
      ...
      for (const f of failures) {
        console.log(`  - ${f.repo}/${f.app}/${f.architecture}/${f.platform} ${f.artifactName}@${f.version}: ${f.reason}`);
      }
    }
  }
}

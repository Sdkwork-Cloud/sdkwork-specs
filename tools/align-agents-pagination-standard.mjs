#!/usr/bin/env node
/**
 * Inserts or refreshes the List And Search Pagination section in AGENTS.md files.
 * See AGENTS_SPEC.md and PAGINATION_SPEC.md.
 */
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { parseArgs } from 'node:util';
import { upsertAgentsPaginationSection, walkAgentsFiles } from './lib/pagination-patterns.mjs';
...
function main() {
  ...
  for (const file of agentsFiles) {
    ...
    const after = upsertAgentsPaginationSection(before);

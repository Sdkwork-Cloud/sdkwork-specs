...
function renderBootstrapRs(applicationCode, mounts, root) {
  const withMount = mounts.filter((item) => item.mount);
  ...
  const paramSets = [
    ...new Set(withMount.map((item) => normalizeMountParams(item.mount.params))),
  ];
  const sharedParams = paramSets.length === 1 ? paramSets[0] : '';
  const needsAsync = withMount.some((item) => item.mount.async);
  const extraUses = new Set();
  for (const item of withMount) {
  ...
  const mountLines = withMount.map((item) => {
    const args = item.mount.paramNames
      .map((name) => {
        if (!sharedParams) {
          return name;
        }
        const occurrences = withMount.filter((other) =>
    ...
    const call = item.mount.async
      ? `${item.libName}::gateway_mount(${args}).await`
      : `${item.libName}::gateway_mount(${args})`;
...
export function materializeGatewayAssembly(root) {
  ...
  if (!preserveBootstrap) {
    writeFileEnsuringDir(bootstrapPath, renderBootstrapRs(applicationCode, mounts, root));
  }

...
function usage() {
  return [
    'Usage: node tools/audit-apps-directory-index-workspace.mjs --workspace <dir> [--prefix sdkwork-]',
    '',
    'Reports apps/README.md compliance for SDKWork repositories with AGENTS.md.',
  ].join('\n');
}
...
for (const repoRoot of repositories) {
  const result = validateAppsDirectoryIndex(repoRoot);
...
if (failures.length > 0) {
  console.error('\nRun alignment:');
  console.error(`node tools/align-apps-directory-index.mjs --workspace ${workspaceRoot}`);
  process.exit(1);
}

...
- A reusable package owns its own default fragments. Product apps may override those fragments during bootstrap or provider composition, but product overrides must keep the same fragment boundaries.
...
- Product override files must be colocated by product package, locale, domain, capability, and fragment. Do not maintain one product-wide override file per locale.
...
- [ ] Product overrides preserve package-local fragment boundaries.

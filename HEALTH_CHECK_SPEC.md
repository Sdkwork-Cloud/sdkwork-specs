...
- Version: 1.0
- Scope: infrastructure liveness, readiness, and metrics HTTP probes for every SDKWork HTTP listener, API server, gateway, and worker with an HTTP admin surface
...
This standard defines the **infrastructure** health surface. It is not the same as domain health APIs such as storage-provider health checks, MCP server health checks, or `/app/v3/api/system/health`.
...
- `src/health.rs` in `sdkwork-<application-code>-api-server` `MUST` contain only readiness assembly (`ReadinessCheck` wiring), not route definitions.
...
- App API system health endpoints
...
- API server crates do not define duplicate local `.route("/healthz")` handlers except documented gateway/upstream extensions

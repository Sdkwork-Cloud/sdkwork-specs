...
| `DEPENDENCY_MANAGEMENT_SPEC.md` | Source/build dependency paths, local development materialization, release Git dependency checkout, cross-platform path rules, and dependency-owned SDK/API boundaries | Adding, changing, or cleaning SDKWork repository dependencies, package workspace paths, local source dependency links, release dependency refs, or cross-platform dependency documentation |
...
| `APP_SDK_INTEGRATION_SPEC.md` | Cross-architecture app SDK integration, application dependency composition, generated language SDK boundaries, Rust backend composition, appbase IAM runtime, and global TokenManager wiring | Wiring generated SDKs into PC React, H5 mobile React, Flutter, mini program, native Android/iOS/Harmony, desktop/native, Rust-enabled apps, appbase IAM/login/session, or dependency SDK composition |
...
| Create a new application | `SDKWORK_WORKSPACE_SPEC.md` | `REQUIREMENTS_SPEC.md`, `APP_MANIFEST_SPEC.md`, `APPLICATION_SPEC.md`, `CONFIG_SPEC.md`, `DEPLOYMENT_SPEC.md`, `QUALITY_GATE_SPEC.md`, `RELEASE_SPEC.md`, `DOCUMENTATION_SPEC.md` |
...
| Create or standardize an application SDK workspace | `SDK_SPEC.md` | `SDK_WORKSPACE_GENERATION_SPEC.md`, `API_SPEC.md`, `COMPONENT_SPEC.md`, `CONFIG_SPEC.md`, `MIGRATION_SPEC.md` when changing existing SDK families, `TEST_SPEC.md` |
| Generate or consume an SDK | `SDK_SPEC.md` | `SDK_WORKSPACE_GENERATION_SPEC.md` for `sdks/` layout and generator artifacts, then `API_SPEC.md`, `CONFIG_SPEC.md`, `FRONTEND_SPEC.md`, `MIGRATION_SPEC.md` when changing existing consumers |
...
- Apps must maintain one global TokenManager per authenticated session context. `sdkwork-appbase` IAM runtime owns login, registration, refresh, current session, logout, runtime metadata, current-user self-service, and shared token propagation to appbase app/backend SDKs, product app/backend SDKs, dependency app/backend SDKs, and approved composed wrappers backed by those SDKs.
- Application bootstrap must build an SDK inventory before feature services are constructed, classify every consumed SDK credential mode, bind authenticated app-api/backend-api SDKs to the single TokenManager closure, and keep protected open-api API-key providers separate.
...
- Source/build dependency paths, local development dependency materialization, release Git dependency checkout, and dependency-owned SDK/API boundaries start from `DEPENDENCY_MANAGEMENT_SPEC.md`.
...
- [ ] Declare source/build dependencies with `DEPENDENCY_MANAGEMENT_SPEC.md`: local dev materializes `.sdkwork/dependencies/<dependency-id>` from relative sibling repositories, while release/CI checks out Git repository refs to the same dependency root.

...
- TypeScript application/backend SDK transports and approved runtime bridges `MUST` delegate dual-token header construction to `@sdkwork/sdk-common` `buildAuthHeaders('dual-token', ..., tokenManager)` instead of reading tokens and formatting `Authorization` or `Access-Token` locally. IAM credential entry uses the first-class `credential-entry-bootstrap` transport profile: it injects bootstrap `Access-Token` only, never adds session/context credentials, and fails before dispatch when bootstrap is absent, as governed by `IAM_CREDENTIAL_ENTRY_SPEC.md`.

...
- `getAuthHeaders()` or the language-equivalent runtime helper may expose `Authorization: Bearer <authToken>`, `Access-Token: <accessToken>`, and optional locale headers for approved runtime bridges. Protected SDK transports must send `Access-Token` whenever an access token is available. UI and feature services must not manually assemble these headers.
